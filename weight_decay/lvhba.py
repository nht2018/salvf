...
for e in range(N_exp):
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, X_train_torch.size(0), batch_size):
            ...
            loss_inner = loss_inner + lamda * constraint  - 0.5 * gamma2 * torch.norm(lamda - z) ** 2
            for name, param in model_inner.named_parameters():
                loss_inner += 0.5 * gamma1 * torch.norm(param - model_outer.state_dict()[name]) ** 2
        ...
        for i in range(0, X_val_torch.size(0), batch_size):
            ...
            for name, param in model_outer.named_parameters():
                loss_between += 0.5 * gamma1 * torch.norm(param - model_inner.state_dict()[name]) ** 2

            loss_outer = (  1 / ck * criterion(logits_val, y_val_batch) +  
                 criterion(logits_val, y_val_batch) -  criterion(logits_train, y_train_batch) - 0.5 * c1 * lamda * constraint_inner
                - 0.5  * gamma1 * loss_between 
                + 0.5  * gamma2 * torch.norm(lamda - z) ** 2 
            )
            ...
            with torch.no_grad():
                sum_param = 0
                for param in model_outer.parameters():
                    param -= alpha_param * param.grad
                    sum_param += torch.norm(param) ** 2
                ...
                if sum_param > C ** 2:
                    norm_param = torch.sqrt(sum_param)
                    coeff = norm_param / C
                    for param in model_outer.parameters():
                        param *= 0.5 * (1 + 1 / coeff)
...
df.to_csv("LVHBA_history.csv", index=False)
