...
for e in range(N_exp):
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, X_train_torch.size(0), batch_size):
            ...
            loss_inner = loss_inner +  lamda * constraint 
        ...
        for i in range(0, X_val_torch.size(0), batch_size):
            ...
            loss_outer = (  criterion(logits_val, y_val_batch) +  
                c1 * criterion(logits_val, y_val_batch) - c1 * criterion(logits_train, y_train_batch) + z * constraint_outer 
            )
...
df.to_csv("BLOOC_history.csv", index=False)
